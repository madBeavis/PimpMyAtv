...
def getRelayStatus(devices, delay, wait, silent=False):
    ...
    if count_devices > 1:
        if not silent:
            print('Collecting status for ' + str(count_devices) + ' devices, please wait.\n')

    for device_status in devices:
        # Send command
        telnet_obj.write(b"relay read " + device_status.encode("ascii") + b"\r\n")
        time.sleep(wait)

        # Read and process response from the device
        response = telnet_obj.read_eager()

        retValueBytes = re.split(br'[&>]', response)[0]
        retValue = str(retValueBytes, 'ascii')

        if 'on' in retValue:
            retValues += '1'
        elif 'off' in retValue:
            retValues += '0'
...
def fixUpDeviceNames(devices):
    ...
    if 'Z' in devices.upper():
        return "0123456789ABCDEFGHIJKLMNOPQRSTUV", False
...
DeviceIP = "192.168.8.5"  # Device IP address
...
if connectToDevice(DeviceIP):
    ...
    if args.reset:  # reset relay module
        ...
        devices_to_power_cycle = getRelayStatusAll()
    ...
    # apply power options
        ...
        returnValue = getRelayStatusAll()
        print(returnValue)
        exit(returnValue)


    elif args.status and my_devices != "":
        returnValue = getRelayStatus(my_devices, args.delay, args.wait, silent=args.silent)

        print("Power Status for Device(s)")
        print("Devices:  " + my_devices)
        print("Status:   " + returnValue[0])
        print("")
